...
class Solution {
    public Node copyRandomList(Node head) {
        ...
        while(node != null) {
            Node newNode;
            if(newHead == null) {
                prev = newHead = newNode = new Node(node.val);
            }
            else {
                prev.next = newNode = new Node(node.val);
                prev = prev.next;
            }

            hashMap.put(node.hashCode(), newNode);
            node = node.next;
        }
        ...
        while(node != null) {
            if(node.random != null) {
                prev.random = hashMap.get(node.random.hashCode());
            }
            node = node.next;
            prev = prev.next;
        }
